...
Prerequisites: just a `hosts.toml` with at least one host. The engine is **bundled** in the
plugin — the launcher below self-bootstraps it into a private venv on first run (one-time, needs
network for PyPI), so this works straight after a plugin install with no separate setup. An
existing PATH install (pipx / `install.sh`) is used as-is. Pick a host with
...
launcher, which resolves the engine for you:
...
- If the launcher's first-run bootstrap fails (offline — no PyPI), say so plainly and fall back
  to the bundled bash harness (`bb-watch.sh`), which needs nothing but ssh; or run the repo's
